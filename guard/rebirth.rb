...
module Guard
  class Rebirth < Guard
    ...
    def initialize(watchers=[], options={})
      ...
      ::Guard::UI.info("Guard::Rebirth #{RebirthVersion::VERSION} setting up test environment... this may take some time", :reset => true)
...
#      @runner = Runner.new(options)
    ...
    def start
      ::Guard::UI.info("Guard::Rebirth #{RebirthVersion::VERSION} is running, with Test::Unit #{::Test::Unit::VERSION}!", :reset => true)
...
#        rescue Exception => ex find what kind of exception to handle first
    ...
    def reload(klass_name, path)
      begin
        ...
        load(path)
        ::Guard::UI.info("#{klass_name} on #{path} rebirth process is done!", :reset => true)
        to_class(klass_name)
      rescue Exception => ex
        puts ex
      end
